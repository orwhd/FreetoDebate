...
class KnowledgeGraphExtractor:
    def __init__(self, knowledge_base_path="./knowledge_base"):
        ...
        # 已处理文件记录
        self.progress_file = os.path.join(knowledge_base_path, "processed_files.txt")
    ...
    def process_data(self, input_data_or_file="data/results.json"):
        """处理数据文件"""
        try:
            if isinstance(input_data_or_file, str):
                data = self.read_json_file(input_data_or_file)
            else:
                data = input_data_or_file
...
def main():
    try:
        import argparse
        parser = argparse.ArgumentParser(description="知识图谱提取器")
        parser.add_argument("input_file", nargs="?", default="data/results.json", help="输入JSON文件路径")
        parser.add_argument("output_dir", nargs="?", default="./knowledge_base", help="知识库输出目录")
        args = parser.parse_args()

        print(f"\n开始处理数据: {args.input_file}")
        print(f"输出目录: {args.output_dir}")
        
        extractor = KnowledgeGraphExtractor(knowledge_base_path=args.output_dir)
        kg = extractor.process_data(args.input_file)
        print("知识图谱创建完成并可视化")
